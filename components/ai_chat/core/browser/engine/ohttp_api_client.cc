...
// Cookie header used to authenticate Leo SKU credentials.
constexpr char kLeoPremiumCookiePrefix[] = "__Secure-sku#brave-leo-premium=";
...
void OHTTPAPIClient::DispatchOhttpRequest(
  ...
  if (credential.has_value()) {
    relay_headers.SetHeader(
        net::HttpRequestHeaders::kCookie,
        base::StrCat({kLeoPremiumCookiePrefix, credential->credential}));
  }
  relay_headers.SetHeader("x-brave-key", BUILDFLAG(BRAVE_SERVICES_KEY));
