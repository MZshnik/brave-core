...
// Picks a random temporary container icon from the available icons.
...
// Picks a random temporary container background color from the available
// colors.
...
bool IsTemporaryContainerId(std::string_view container_id) {
  return container_id.starts_with(kTemporaryContainerIdPrefix);
}
...
mojom::ContainerPtr CreateTemporaryContainer() {
  ...
  std::string name = GenerateTemporaryContainerName();
  mojom::Icon icon = PickTemporaryContainerIcon();
  SkColor background_color = PickTemporaryContainerBackground(*color_provider);

  return mojom::Container::New(std::move(id), std::move(name), icon,
                               background_color);
}
