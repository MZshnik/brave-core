...
inline constexpr char kTemporaryContainerIdPrefix[] = "t-";

// Checks if a given container id is a temporary container id.
bool IsTemporaryContainerId(std::string_view container_id);

// Builds a local-only temporary container (`t-` id, BIP-39 two-word name,
// predefined background color, default icon).
